...
";
    ...
    macro_rules! test {
        ( $x:expr ) => {{
            $x.timeout(Duration::from_secs(TIMEOUT_DURATION))
                .await
                .map_err(|_err| {
                    anyhow::anyhow!("Test timeout after {}s at {}:{}", TIMEOUT_DURATION, file!(), line!())
                })?
        }};
    }
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn case_10_first_game_state() -> anyhow::Result<()> {
        let db_path = get_random_db_path();
        let (_, send_stop, game_thread, port) = test!(bootstrap(&db_path, false))?;
        let mut client = test!(bot::connect_plain("localhost", port))?;
        test!(client.login("test213"))?;
        let _game_state = test!(client.next_game_state())?;
        send_stop.send(())?;
        test!(game_thread)??;
        Ok(())
    }
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn case_11_test_first_game_state() -> anyhow::Result<()> {
        let db_path = get_random_db_path();
        let (_, send_stop, game_thread, port) = test!(bootstrap(&db_path, false))?;
        let mut client = test!(bot::connect_plain("localhost", port))?;
        test!(client.login("test213"))?;
        let game_state = test!(client.next_game_state())?;
        match game_state {
            Game::Player(player_state) => {
                assert!(player_state.coords[0].is_normal());
                assert!(player_state.coords[1].is_normal());
                assert!(player_state.coords[2].is_normal());
            }
            _ => assert!(false),
        }
        send_stop.send(())?;
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn case_12_move() -> anyhow::Result<()> {
        let db_path = get_random_db_path();
        ...
        let game_state = client.next_game_state().await?;

        let mut coords = if let Game::Player(player_state) = game_state {
            player_state.coords
        } else {
            unreachable!();
        };
        {
            ...
            let coords_later = client.until_player_state().await?.coords;
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn case_13_ping_pong() -> anyhow::Result<()> {
        let db_path = get_random_db_path();
        let (_, send_stop, game_thread, port) = test!(bootstrap(&db_path, false))?;
        let mut client = test!(bot::connect_plain("localhost", port))?;
        test!(client.login("test213"))?;
