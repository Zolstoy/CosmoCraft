...
impl App {
    ...
    pub async fn run<S: AsyncRead + AsyncWrite + Unpin>(
        ...
        while !self.should_quit {
            tokio::select! {
                ...
                Ok(game_state) = client.next_game_state() => {
                    match game_state {
                        Game::Player(_player_state) => {
    ...
    fn draw(&mut self, f: &mut Frame) {
        ...
        let rows = self.bodies.iter().enumerate().map(|(i, data)| {
            let color = match i % 2 {
                0 => Color::Black,
                _ => Color::DarkGray,
            };
        ...
        let mut scrollbar_state = ScrollbarState::new(self.bodies.len()).position(self.list_scroll);
        ...
        if self.star.id != u32::default() {
            ...
            let system_canvas = Canvas::default()
                ...
                .y_bounds(y_bounds)
                .paint(move |ctx| {
                    for bodies in cln.values() {
                        let mut coords = bodies.coords;
                        coords[0] -= self.star.coords[0];
                        coords[2] -= self.star.coords[2];

                        match bodies.body_type.to_string().as_str() {
                            "1" => {
                                ctx.layer();
                                ctx.draw(&Circle {
                                    x: coords[0],
            ...
            Event::Mouse(event) => match event.kind {
                ...
                MouseEventKind::ScrollDown => {
                    ...
                    if self.list_area.contains(Position::new(event.column, event.row)) {
                        self.list_scroll += 1;
                        if self.list_scroll >= self.bodies.len() {
                            self.list_scroll = self.bodies.len().saturating_sub(1);
                        }
                    }
                }
                _ => {}
