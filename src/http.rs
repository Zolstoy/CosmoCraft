    ...
    T: tokio::io::AsyncRead + tokio::io::AsyncWrite + std::marker::Unpin + std::marker::Send + 'static,
{
    ...
    tokio::task::spawn(async move {
        ...
        let result = http1::Builder::new()
            ...
            .await;
        if result.is_err() {
            cosmocraft_log!(warn, address, "Serve http connection error: {}", result.err().unwrap());
        } else {
            cosmocraft_log!(info, address, "Served http connection");
        }
    });
}
...
pub async fn serve(
    ...
    *response.status_mut() = StatusCode::BAD_REQUEST;
    ...
    if hyper_tungstenite::is_upgrade_request(&request) {
        cosmocraft_log!(info, address, "Upgrade request");
        ...
        if res.is_err() {
            ...
            *response.body_mut() = Full::<Bytes>::new(format!("Can't upgrade to websocket: {}", err_str).into());
            ...
            cosmocraft_log!(info, address, "WS upgrade error");
            return Ok(response);
        }
        ...
        *response.body_mut() = Full::<Bytes>::new(format!("Websocket only").into());
        cosmocraft_log!(info, address, "HTTP non WS request");
        return Ok(response);
    }
}
